...
function main() {
    ...
    Request = {
        serie:DefaultSerie,
        DateStart: new Date(Date.now() - 28 * (24 * 60 * 60 * 1000)),
        ...
        () => {
            ...
            return NewSeries.map(
                Curve => {
                    ...
                    Curve.line = d3.line()
                        // .defined(function (d) { console.log('d',d);return d; })
                        // .defined(function (d) { console.log('d[1] !== null',d);return d[1] !== null; })
                        // .defined(function (d) { console.log('d.y !== null',d);return d.y !== null; })
                        // .defined(function (d) { console.log('d.y',d);return d.y; })
                    ...
                    Curve.xPosOfYAxis = axisBuild(Request.yScales, Curve.Unit, Curve.Count);
                    ...
                    Curve.domLine.datum(Curve.data)
                        .attr("z-index", -1)
                        .attr("fill", "none")
...
function GetDataFull(serie) {
    console.log('GetDataFull serie', serie);
    serie.Color = {
    ...
    .then( // Construction des Axes X et raprochement en cas d'unite similaire
        Serie => {
            if (!Request.yScales[Serie.Unit]){ // si l'axe Y de cette unite n'existe pas
                ...
                Request.yScales[Serie.Unit].update.axes
                        // .transition()
                        // .duration(1000)
                        .call(Request.yScales[Serie.Unit].update.axisSide(Request.yScales[Serie.Unit]))
                        .call(g => g.selectAll(".tick text").append("tspan").text(Serie.Unit))
                Request.serie
                    .filter(s => s.Unit == Serie.Unit)
                    .map(function(Curve) {
                        console.log('Curve',Curve)
                        Curve.domLine
                            .transition()
...
function axisBuild(yAxes, Unit, Count){
    switch (Count) {
        case 0:
            // Add the y Axis Left oriented Left
            yAxes[Unit].update = {
                axes : (svg.append("g")
                    .call(d3.axisLeft(yAxes[Unit]))),
                    // sideNumber : Count,
                    axisSide : d3.axisLeft,
                    xPosOfYAxis : 0,
                }
            xPosOfYAxis = 0;
            break;
        case 1:
            ...
            yAxes[Unit].update = {
                axes : (svg.append("g")
                    ...
                    .call(d3.axisRight(yAxes[Unit]))),
                axisSide : d3.axisRight,
                xPosOfYAxis : width,
            }
            xPosOfYAxis = width;
            break;
    ...
    yAxes[Unit].update.axes
        .call(g => g.selectAll(".tick text").append("tspan").text(Unit))
    return xPosOfYAxis;
}
